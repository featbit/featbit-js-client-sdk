...
const specialValue = '___071218__';
...
class AutoCapture {
  ...
  private async bindClickHandlers(exptMetricSettings: IExptMetricSetting[]) {
    ...
    if (!!clickSetting) {
      ...
      nodes.forEach(node => {
        node['datafbeventname'] = clickSetting.eventName;
    ...
    for(let zeroCodeSetting of zeroCodeSettings) {
      ...
      if (zeroCodeSetting.featureFlagType === FeatureFlagType.Pretargeted) {
        ...
        if (!!effectiveItems && effectiveItems.length > 0) {
          const result = Featbit.variation(zeroCodeSetting.featureFlagKey, specialValue);
  
          if (result !== specialValue) {
            this.applyRules(effectiveItems, result);
          }

          Featbit.on(`ff_update:${zeroCodeSetting.featureFlagKey}`, () => {
            const result = Featbit.variation(zeroCodeSetting.featureFlagKey, specialValue);
            if (result !== specialValue) {
              this.applyRules(effectiveItems, result);
            }
  ...
  private revertRules (items: ICssSelectorItem[]) {
    ...
    nodes.forEach(node => {
      ...
      const rawStyle = node.getAttribute(`data-fb-${specialValue}`);
      ...
      if (variationValue !== ffResult) {
        ...
        nodes.forEach(node => {
          ...
          if (left !== '-99999px') {
            ...
            node.setAttribute(`data-fb-${specialValue}`, JSON.stringify(style));
